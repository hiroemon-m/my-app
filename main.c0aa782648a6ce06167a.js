        ...
        try {
          const promises = (topic || ["default_topic"]).map(async topicId => {
            ...
            Object.entries(yearToPersona).forEach(_ref2 => {
              ...
              // 著者データを出現回数でソート
              const sortedAuthors = Object.entries(yearData).sort((_ref3, _ref4) => {
                let [, countA] = _ref3;
                let [, countB] = _ref4;
              ...
              personaData[`persona_${personaNum}`] = {
                authors: sortedAuthors.map(_ref5 => {
                  let [author, count] = _ref5;
  ...
  (0,react__WEBPACK_IMPORTED_MODULE_0__.useEffect)(() => {
    if (update && preparedData) {
      const plotData = preparedData.searchList.map((k, j) => {
        ...
        const hoverTexts = ["1", "2", "3", "4", "5"].map((num, idx) => {
          if (k === "株式会社熊谷組" && authorData[currentTopic] && authorData[currentTopic][`persona_${num}`]) {
            ...
            return `ペルソナ${num}<br>著者: ${authorInfo.authors || '出願者なし'}`;
          }
          return `ペルソナ${num}`;
        });
        return {
